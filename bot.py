...
def handle_india_number_response(message):
    ...
    try:
        # Updated API endpoint with proper logging
        api_url = f"http://osintx.info/API/krobetahack.php?key=SHAD0WINT3L&type=mobile&term={num}"
        ...
        logger.info(f"Response Content (first 500 chars): {r.text[:500]}")
        ...
        if r.status_code != 200:
            ...
            return bot.send_message(message.chat.id, f"❌ API request failed with status code: {r.status_code}. Try again later.")
        
        # Check if response is empty
        if not r.text or r.text.strip() == "":
            logger.error("API returned empty response")
            refund_credit(message.from_user.id)
            return bot.send_message(message.chat.id, "❌ API returned empty response. Please try again later.")
        ...
        try:
            ...
            refund_credit(message.from_user.id)
            return bot.send_message(message.chat.id, "❌ Invalid API response format. Server returned non-JSON data.")
        ...
        if isinstance(response_json, dict):
            ...
            if 'data' in response_json:
                ...
                # If no 'data' key, use the entire dict as a single record
        ...
        for i, rec in enumerate(data_list, 1):
            try:
                ...
                if isinstance(rec, dict):
                    ...
                    # Try to handle string or other formats
                    try:
                        rec_str = str(rec)
                        out = f"""
📋 <b>Record #{i}</b>
...
"""
                        ...
                        logger.error(f"Error processing non-dict record #{i}: {e}")
                        continue
...
"""
        ...
        try:
            ...
            pass
        refund_credit(message.from_user.id)
        bot.send_message(message.chat.id, f"⚠️ Error: <code>{str(e)}</code>")
...
def handle_aadhar(m):
    try:
        ...
        try:
            ...
            api_url = f"http://osintx.info/API/krobetahack.php?key=SHAD0WINT3L&type=id_number&term={aid}"
            logger.info(f"Making request to: {api_url}")
            ...
            r = requests.get(api_url, headers=headers, timeout=300)
            ...
            if r.status_code != 200:
                ...
                logger.error(f"Response content: {r.text}")
                refund_credit(m.from_user.id)
                return bot.send_message(m.chat.id, f"❌ API request failed with status code: {r.status_code}. Please try again later.")
            
            # Check if response is empty
            if not r.text or r.text.strip() == "":
                logger.error("API returned empty response")
                refund_credit(m.from_user.id)
                return bot.send_message(m.chat.id, "❌ API returned empty response. Please try again later.")
            ...
            try:
                ...
                logger.error(f"Raw response: {r.text}")
                refund_credit(m.from_user.id)
                return bot.send_message(m.chat.id, "❌ Could not parse API response. The server returned non-JSON data.")

            # Handle different response formats
            if isinstance(data, dict):
                # If response is a dictionary, check if it has a 'data' key
                if 'data' in data:
                    data_list = data['data']
                else:
                    # If no 'data' key, treat the entire dict as a single record
                    data_list = [data]
...
"""
            ...
            if not formatted_records:
                refund_credit(m.from_user.id)
                return bot.send_message(m.chat.id, "📭 No valid Aadhaar Data Found!")
            
            footer = f"""
✅ <b>Search completed!</b> Found {len(formatted_records)} record(s)
💳 <b>Credits Used:</b> 1
"""
